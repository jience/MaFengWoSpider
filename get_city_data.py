...
# 执行函数
# city_list = pd.read_excel('城市编号.csv')
...
bar = Bar("游记总数量TOP10")
bar.add("游记总数", attr, v1, is_stack=True, is_more_utils=True)
bar.print_echarts_options()  # 方便调试
bar.render('游记总数量TOP10.html')

# 地理坐标图
data = [(city_base['city_name'][i], int(city_base['total_city_yj'][i])) for i in range(0, 11)]
